...
public class Controller {
    ...
    private static final String DB_URL = "jdbc:sqlite:space_saver.db";
    ...
    public boolean enterVehicle(String vehicleNum) {
        try (Connection conn = connect()) {
            ...
            PreparedStatement findSlot = conn.prepareStatement(
                    "SELECT slotID FROM ParkingSlots WHERE slotType = ? AND isOccupied = 0 LIMIT 1");
            ...
            PreparedStatement ticketStmt = conn.prepareStatement(
                    "INSERT INTO Tickets(entryTime, vehicleNum) VALUES (?, ?)");
            ticketStmt.setString(1, LocalDateTime.now().toString());
            ticketStmt.setString(2, vehicleNum);
            ticketStmt.executeUpdate();

            PreparedStatement occupySlot = conn.prepareStatement(
                    "UPDATE ParkingSlots SET isOccupied = 1 WHERE slotID = ?");
    ...
    public boolean exitVehicle(String vehicleNum) {
        try (Connection conn = connect()) {
            PreparedStatement ticketStmt = conn.prepareStatement(
                    "SELECT ticketID, entryTime FROM Tickets WHERE vehicleNum = ? AND exitTime IS NULL ORDER BY ticketID DESC LIMIT 1");
            ...
            PreparedStatement updateTicket = conn.prepareStatement(
                    "UPDATE Tickets SET exitTime = ?, duration = ?, paymentID = ? WHERE ticketID = ?");
            ...
            PreparedStatement vehicleStmt = conn.prepareStatement(
                    "SELECT vehicleType FROM Vehicles WHERE vehicleNum = ?");
            ...
            if (vehicleRs.next()) {
                ...
                PreparedStatement freeSlot = conn.prepareStatement(
                        "UPDATE ParkingSlots SET isOccupied = 0 WHERE slotType = ? AND isOccupied = 1 LIMIT 1");
